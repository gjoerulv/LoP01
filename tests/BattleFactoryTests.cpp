...
std::filesystem::path BuildBattleFactoryTestContent(const std::string& folderName) {
    ...
    WriteTextFile(root / "battle_scenarios.json", R"({"battle_scenarios":[{"id":"test_scenario","name":"Test Scenario","seed":7,"allies":[{"unit_id":"hero_player"},{"unit_id":"hero_mira"}],"enemies":[{"unit_id":"enemy_captain"},{"unit_id":"unit_longbow"}]}]})");
...
TEST_CASE("ActivePartyOverride_PreservesOrderAndDuplicates") {
    ...
    const auto allies = CollectUnitIdsBySide(battle->Units(), gameplay::battle::TeamSide::Allies);
    REQUIRE(allies == std::vector<std::string>{"unit_guard", "unit_medic", "unit_guard"});

    std::filesystem::remove_all(root);
}

TEST_CASE("ActivePartyGenericOnly_AllowsBattleWithNoPlayerCharacterAlly") {
