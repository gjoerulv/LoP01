...
Milestone sequencing, latest-completed status, and selected next milestone live in `docs/implementation_roadmap.md`. Do not duplicate milestone state in this scope file.

## 2. v3 target

v3 targets a playable authored Scenario/Campaign experience built on the v1/v2 systems. The target loop is:

1. the player starts from a shell/menu flow;
2. chooses Campaign, Standalone Scenario, or Tutorial content where supported;
...
8. the Scenario can reach a clear victory/defeat/result state; and
...
- Do not keep expanding v2. v2 is complete and should remain historical context.

## 4. v3 scope areas

The active roadmap may select these areas in any order after auditing the post-milestone codebase:
...
3. **Threat Preview + Auto-Resolve Foundation.** Add cheap threat preview for Region/service battles and a bounded auto-resolve path that uses or aligns with the existing battle rules.
...
## 6. Relationship to v2/v3 stand-ins

v3 should replace the most consequential stand-ins only when a milestone selects them:
...
- shell flow remains incomplete relative to `game_shell_flow.md`.
...
`docs/implementation_roadmap.md` is the active milestone plan and the only place that should record current milestone state. Archived docs are historical context only. Do not use archived roadmap/scope files as current requirements unless comparing history. Milestone-agnostic docs remain source-of-truth for final rules. Update them only when the actual long-term rule, schema, terminology, or architectural direction changes.
