...
Trader Services may author ownership-tier curves for prices or exchange rates. Curves are per service type, capped at 8 owned Services for the owning team. Trading Post exchange may use an authored matrix per tier. A Trading Post `exchangeMatrix` is non-Gold resource-for-resource barter only: Gold may not appear as `from` or `to`. Trading Post Gold buy/sell uses base values plus the tier `priceFactor` (a basis-100 Gold-trade favorability scalar; `100` = default rates, `> 100` favors the player), never the matrix. Effective tier 0 is data-driven: a usable Trading Post the player does not own (allied-, enemy-, or neutral-owned, or unowned) resolves at effective tier 0; an authored tier 0 applies if present, otherwise built-in defaults apply; ownership raises effective tier above 0 only when the exact used service is player-owned and eligible. Legal default curves should exist for every trader service type, so omitted curves load to safe defaults. Service state in content is initial state. Runtime state belongs in save data.
