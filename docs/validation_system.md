...
M32 adds two authored start-state surfaces to the thin scenario, both strictly validated at load:

- **Scenario Context** (`regions` array): not an array (`SCENARIO_REGIONS_TYPE_INVALID`); a non-string/empty entry (`SCENARIO_REGION_FIELD_INVALID`); an entry referencing an unknown Region (`SCENARIO_REGION_UNKNOWN`); a duplicate Region (`SCENARIO_REGION_DUPLICATE`); and, when a context is authored, a start Region not contained in it (`SCENARIO_START_REGION_NOT_IN_CONTEXT`). An empty/absent list is legal (default context = all Regions).
- **Starting roster** (`playerStart.roster`): `roster` or an `active`/`reserve` value not the expected type (`SCENARIO_START_ROSTER_TYPE_INVALID`); an entry that is not an object (`SCENARIO_START_ROSTER_ENTRY_TYPE_INVALID`); a missing/empty/non-string `unitId` (`SCENARIO_START_ROSTER_FIELD_INVALID`); a non-positive or non-integer `quantity` (`SCENARIO_START_ROSTER_QUANTITY_INVALID`); an unknown unit id (`SCENARIO_START_ROSTER_UNIT_UNKNOWN`); a hero/leader unit with quantity ≠ 1 (`SCENARIO_START_ROSTER_HERO_QUANTITY_INVALID`); a hero appearing more than once across both lists (`SCENARIO_START_ROSTER_HERO_DUPLICATE`, which also catches an illegally duplicated Player Character); an active list with no leader-capable unit (`SCENARIO_START_ROSTER_NO_ACTIVE_LEADER`); more entries than the active (5) / reserve (8) slot capacities (`SCENARIO_START_ROSTER_ACTIVE_OVERFLOW` / `SCENARIO_START_ROSTER_RESERVE_OVERFLOW`); and, when the unit catalog defines a Player Character, a roster that does not include it exactly once (`SCENARIO_START_ROSTER_PLAYER_CHARACTER_MISSING`). Reachability/softlock proofs and authored hero-pool validation remain deferred.
...
Owned mine/resource-service validation enforces: service ids are globally unique and each location is placed in at most one Region node (so a service id is a stable owned-service instance key); mine outputs reference valid resources with positive amounts and no duplicate output resource per service; a mine service must define outputs and a non-mine service must not. Unit passive effects (`passive_effects`) are validated per entry: a supported kind; a `mine_production` effect needs a valid resource, positive amount, and target `mine`; a `leader_energy` effect needs a positive amount and no resource/target. Malformed `passive_effects` (not an array, or a non-object entry) and authoring both `passive_effects` and the legacy `mine_production_passive` on one unit are rejected; the legacy key is converted into a canonical `mine_production` entry at load. The runtime rule that only the strongest applicable stationed passive counts per owned service instance and output resource is preserved by the production calculation; stationed references must be stack-backed (a live roster stack whose unit matches).
