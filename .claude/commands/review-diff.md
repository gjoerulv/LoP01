...
- Reject changes that regress M25 stationing invariants, M26 player-side claiming semantics, M27 read-only overview semantics, or M28 storage invariants unless a later roadmap explicitly replaces them.
...
- For M29 diffs, require explicit warning/confirmation before traveling generic stacks are lost, confirmed removal through `GameSession`, stored units unaffected, and no broad service-defense/capture/loss scope creep.
