...
- Service attacks are node-level against player-owned attackable services. Player absent uses the M33 deterministic battle-rule-aligned auto-resolve (`gameplay/battle/AutoResolve`) for the hold/capture decision; player present uses the existing interactive battle surface. M30's `ServiceDefenseRules` power helpers remain as the cheap threat-preview proxy and the undefended-node gate.
- Auto-resolve (M33) is deterministic and RNG-free: it reuses the interactive battle's physical-damage formula + leader aura via the shared `BattleUnitConversion`, with expected (average) damage, agility CTB ordering, and focus-weakest targeting; basic physical attacks only (no skills/items/magic/status/escape/revive/AI choices). Defenders win CTB ties. The Player Character never contributes to defense (`BuildNodeDefenderForce` skips it) and is never lost/TU. This is a foundation, not the final full-fidelity auto-resolve.
- Threat preview (M33) is pure/read-only and reveal-gated: `GameSession::ThreatPreviewForNode` / `ServiceDefensePreview` return a bounded band (Low/Even/Dangerous/Overwhelming, `gameplay/battle/ThreatPreview`) from a cheap battle-stat power ratio — the §16 cheap estimate, intentionally distinct from the auto-resolve. Unrevealed/hidden enemies never leak through preview. Surfaced as a bounded estimate on revealed hostile Region nodes and on contested owned-service overview rows; the owned-service overview stays read-only.
