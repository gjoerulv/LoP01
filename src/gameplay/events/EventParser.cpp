...
std::string ReadTriggerTargetId(EventTriggerType type, const nlohmann::json& t)
{
...
bool IsKnownConditionLeafType(const std::string& t)
{
    return t == "always" || t == "teamHasResource" || t == "teamHasHero" || t == "storyFlagSet";
}

bool IsKnownActionType(const std::string& t)
{
    ...
    if (doc.contains("eligibility") && doc["eligibility"].is_object()) {
        ...
        if (e.contains(tcKey) && e[tcKey].is_array()) {
            for (const auto& c : e[tcKey]) {
                if (c.is_string()) {
                    def.eligibility.teamColors.push_back(c.get<std::string>());
                }
            }
        }
        ...
        if (e.contains(tkKey) && e[tkKey].is_array()) {
            for (const auto& k : e[tkKey]) {
                if (k.is_string()) {
                    def.eligibility.teamKinds.push_back(k.get<std::string>());
                }
            }
        }
        ...
        if (e.contains(rhKey) && e[rhKey].is_array()) {
            for (const auto& h : e[rhKey]) {
                if (h.is_string()) {
                    def.eligibility.requiredHeroIds.push_back(h.get<std::string>());
                }
            }
        }
    }
...
std::vector<ValidationMessage> ValidateEventDefinition(const nlohmann::json& doc)
{
    ...
    if (!doc.contains("id") || !doc["id"].is_string()) {
        ...
        msgs.push_back({Severity::Error, "EVENT_ID_EMPTY", "id", "Event \"id\" must not be empty.", ""});
    }
            ...
            for (size_t i = 0; i < doc["actions"].size(); ++i) {
                ...
                if (!a.contains("type") || !a["type"].is_string()) {
                    msgs.push_back({Severity::Error, "EVENT_ACTION_TYPE_MISSING", path + ".type",
                                    "Action at index " + std::to_string(i) +
                    ...
                    msgs.push_back({Severity::Error, "EVENT_ACTION_TYPE_UNKNOWN", path + ".type",
                                    "Action type \"" + a["type"].get<std::string>() +
                                        "\" is not recognised. " +
                                        std::string(KnownActionTypesText()),
                                    ""});
                }
            }
        }
